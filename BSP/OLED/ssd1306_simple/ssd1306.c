...
#error "Only 32, 64, or 128 lines of height are supported!"
    ...
    ssd1306_WriteCommand(0x14);   // 电荷泵常开
}
...
{
    /* 1. 设置显存起始地址（水平模式已初始化，这里直接发数据） */
    ssd1306_WriteCommand(0x40); // 列地址=0，页地址=0（上电复位值）
    /* 2. 整 1024 B DMA 发送 */
    HAL_I2C_Mem_Write_DMA(&SSD1306_I2C_PORT,
                          SSD1306_I2C_ADDR,
                          0x40,                    // 控制字节 = 0x40（数据模式）
                          I2C_MEMADD_SIZE_8BIT,
                          SSD1306_Buffer,          // 连续 1024 B
                          SSD1306_BUFFER_SIZE);    // 128*64/8
    /* 3. 等待完成（阻塞式，最简单） */
